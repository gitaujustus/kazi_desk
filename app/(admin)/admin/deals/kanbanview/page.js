...
import DealsKanBanView from '@/components/leads/dealsKanbanView';
import AddDealInfoForm from '@/components/deals/addDealInfo';
...
function Deal() {
  ...
  const addNewStageRef = useRef(null);

  useEffect(() => {
    function handleClickOutside(event) {
      if (addNewStageRef.current && !addNewStageRef.current.contains(event.target)) {
        setShowAddNewStage(false);
      }
    }
  ...
  return (
    <main className='bg-white rounded-lg mx-5 border border-black min-h-[100vh] relative mb-4 pb-4 relative'>
        <section className='bg-[#172448] rounded-t-lg px-3'>
            ...
            <Link href={'/admin/deals'} className={`${pathname === '/admin/deals/kanbanview'? 'bg-[#3D50FC] text-white':'bg-white'} flex gap-2   items-center rounded-lg p-2`}>
                <span>
                    <svg width="24"  height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <button 
              onClick={() => setShowAddNewStage(true)} 
              className={`${showAddNewStage ? 'bg-[#3D50FC]':'bg-white text-black'} flex gap-2 items-center rounded-lg p-2`}
            >
              <span>
                <svg width="24"  height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M19 12.998H13V18.998H11V12.998H5V10.998H11V4.99805H13V10.998H19V12.998Z" fill="black"/>
    ...
    <section>
      ...
      {showAddNewStage && (
        <div className='fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50'>
          <div ref={addNewStageRef}>
            <AddNewStage onClose={() => setShowAddNewStage(false)} />
          </div>
        </div>
      )}
